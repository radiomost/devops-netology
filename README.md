...
```markdown
- Все каталоги с именем `.terraform/` в любом месте проекта.
- Все файлы с расширением `.tfstate` или с именем, содержащим `.tfstate.` (`*.tfstate`, `*.tfstate.*`).
- Все файлы с расширением `.tfvars` и `.tfvars.json` (`*.tfvars`, `*.tfvars.json`).
- Файлы с точными именами `override.tf` и `override.tf.json`.
- Файлы, имя которых оканчивается на `_override.tf` или `_override.tf.json`.
- Файлы с именем `.terraform.tfstate.lock.info`.
- Файлы с именами `.terraformrc` и `terraform.rc`.
- Файлы с именами `crash.log` и `crash.*.log`.
- Все файлы, соответствующие шаблону `*tfplan*` (если строка раскомментирована).
- Другие файлы, указанные в `.gitignore` через отрицательные паттерны (`!example_override.tf`), если они используются.
```
...
git clone https://github.com/radiomost/devops-netology.git
```
Первоначальные изменения файла README.md которые были сделаны в процессе выполнения работы:
...
В репозитории игнорируются следующие файлы и каталоги:
